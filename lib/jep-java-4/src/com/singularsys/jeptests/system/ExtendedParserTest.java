...
public class ExtendedParserTest extends JepTest {
	...
	public void setUp() {
		...
		/*
		 * cp.addGrammerMatcher(new IfThenElseGrammerMatcher( cp.getSymbolToken("IF"),
		 * cp.getSymbolToken("THEN"), cp.getSymbolToken("ELSE"), new If()));
		 */
		jep.setComponent(cp);
	...
	@Ignore("Complex tests not implemented for ExtendedParser")
	@Override
	@Test
	public void testComplex() throws Exception {
	}

	@Ignore("Function tests not implemented for ExtendedParser")
	@Override
	@Test
	public void testFunction() throws Exception {
	}

	@Ignore("3++2 tests not implemented for ExtendedParser as confused with increment/decrement ops")
	@Override
	@Test
	...
	@Ignore("Logical tests not implemented for ExtendedParser")
	@Override
	@Test
	...
	@Ignore("Tests with ^ for power not implemented for ExtendedParser")
	@Override
	@Test
	public void testX2Y() throws Exception {
	}

	@Ignore("String tests not implemented for ExtendedParser")
	@Override
	@Test
	...
	public void testSuffix() throws Exception {
		...
		assertEquals(ot.getOperator(ExtendedOperatorSet.ExtendedOperators.PERCENTKEY), n.getOperator());
		assertEquals(ot.getOperator(ExtendedOperatorSet.ExtendedOperators.FACTKEY), n.jjtGetChild(0).getOperator());
		// can't actually evaluate this a factorial returns a long
	}

	/**
	 * Test a combination of suffix operations with function
	 * for better coverage
	 * 
	 * @throws Exception
	 */
	@Test
	public void prefix_suffix_operators_inside_single_arg_function_parses_correctly() throws Exception {
		Node n0 = jep.parse("abs ++5!%");
		
		OperatorTable2 ot = (OperatorTable2) jep.getOperatorTable();
		
		assertTrue(n0 instanceof ASTFunNode);
		Node n1 = n0.jjtGetChild(0);
		assertEquals(ot.getOperator(JavaOperatorTable.JavaOperators.PREINC), n1.getOperator());
		
		Node n2 = n1.jjtGetChild(0);
		assertEquals(ot.getOperator(ExtendedOperatorSet.ExtendedOperators.PERCENTKEY), n2.getOperator());
		assertEquals(ot.getOperator(ExtendedOperatorSet.ExtendedOperators.FACTKEY), n2.jjtGetChild(0).getOperator());
		// can't actually evaluate this a factorial returns a long
	}
	...
	@Test
	@Ignore("MacroFunction test not implements for ExtendedParser")
	@Override
	public void testRecursiveMacroFunction() throws Exception {
	}

	@Test
	@Ignore("!X Y test not implemented for ExtendedParser")
	@Override
	public void test_not_X_Y() throws Exception {
	}

	@Test
	public void list_or_bracket_matcher_parses_bracketed_expressions() throws Exception {
		valueTest("(5)", 5.0);
		valueTest("(5+4)", 9.0);
		valueTest("3*(5+4)", 27.0);
		valueTest("(5+4)*3", 27.0);
		valueTest("3(5+4)", 27.0);
		valueTest("(5+4)3", 27.0);
		valueTest("3 (5+4)", 27.0);
		valueTest("(5+4) 3", 27.0);
		valueTest("x=3", 3.0);
		valueTest("x (5+4)", 27.0);
		valueTest("(5+4) x", 27.0);
		valueTest("x(5+4)", 27.0);
		valueTest("(5+4)x", 27.0);
		valueTest("x+(5+4)", 12.0);
		valueTest("(5+4)+x", 12.0);		
		valueTest("(5+4)(1+2)", 27.0);
		valueTest("(1+2)*(5+4)", 27.0);
		valueTest("(1+2) (5+4)", 27.0);
		valueTest("(1+2)(5+4)(1+2)", 81.0);

		valueTestString("3*(4,3,2,1)", buildVector(12.0, 9.0, 6.0, 3.0).toString());
		valueTestString("(4,3,2,1)*3", buildVector(12.0, 9.0, 6.0, 3.0).toString());
		valueTestString("3 (4,3,2,1)", buildVector(12.0, 9.0, 6.0, 3.0).toString());
		valueTestString("(4,3,2,1) 3", buildVector(12.0, 9.0, 6.0, 3.0).toString());
		valueTestString("3(4,3,2,1)", buildVector(12.0, 9.0, 6.0, 3.0).toString());
		valueTestString("(4,3,2,1)3", buildVector(12.0, 9.0, 6.0, 3.0).toString());
		valueTestString("x(4,3,2,1)", buildVector(12.0, 9.0, 6.0, 3.0).toString());
		valueTestString("(4,3,2,1)x", buildVector(12.0, 9.0, 6.0, 3.0).toString());
		valueTestString("(1+2)(4,3,2,1)", buildVector(12.0, 9.0, 6.0, 3.0).toString());
		valueTestString("(4,3,2,1)(1+2)", buildVector(12.0, 9.0, 6.0, 3.0).toString());
		valueTest("atan2(1,1)", Math.PI/4,1e-9);
		valueTest("(1+2)atan2(1,1)", 3*Math.PI/4,1e-9);
		valueTest("atan2(1,1)(1+2)", 3*Math.PI/4,1e-9);
	}
